...
namespace api {
	...
	Point2PointConstraintComponent::Point2PointConstraintComponent(const int64_t id, const api::attributeMap & params) : Component(id, params), _connected(false), _selfIdentifier(), _targetIdentifier(), _selfOffset(), _targetOffset(), _targetID() {
			...
			if (p2p != nullptr) {
				if (p2p->_targetIdentifier == _selfIdentifier && p2p->_selfIdentifier == _targetIdentifier) {
					if (!p2p->_connected) {
						...
						EngineController::GetSingletonPtr()->getMessagingFacade()->deliverMessage(boost::make_shared<GameMessage>(messages::PhysicsNodeMessageType, physics::PhyP2PConstraint, core::Method::Create, new physics::Physics_P2PConstraint_Create(_objOwnerID, go->getID(), _selfOffset, _targetOffset), core::Subsystem::Object));
						_targetID = go->getID();
					} else {
	...
	void Point2PointConstraintComponent::Finalize() {
		EngineController::GetSingletonPtr()->getMessagingFacade()->deliverMessage(boost::make_shared<GameMessage>(messages::PhysicsNodeMessageType, physics::PhyP2PConstraint, core::Method::Delete, new physics::Physics_P2PConstraint_Delete(_objOwnerID, _targetID), core::Subsystem::Object));
	}
