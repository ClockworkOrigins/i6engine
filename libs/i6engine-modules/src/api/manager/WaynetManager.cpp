		...
		for (auto & p : _waypoints) {
			for (auto & e : p.second) {
				if (edges.find(std::make_pair(p.first, e.targetWP)) == edges.end()) {
					EngineController::GetSingletonPtr()->getMessagingFacade()->deliverMessage(boost::make_shared<GameMessage>(messages::GraphicsMessageType, graphics::GraLine, core::Method::Create, new graphics::Graphics_Line_Create(e.startPosition, e.targetPosition, Vec3(255.0, 255.0, 255.0)), core::Subsystem::Object));
