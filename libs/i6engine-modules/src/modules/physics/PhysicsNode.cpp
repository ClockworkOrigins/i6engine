...
namespace modules {
	...
	PhysicsNode::PhysicsNode(const int64_t id, const int64_t compId, const Vec3 & position, const Quaternion & rotation, const Vec3 & scale, const api::CollisionGroup & cg, const api::attributeMap & params, api::PhysicalStateComponent::ShatterInterest shatterInterest, PhysicsManager * pm) : _manager(pm), _id(id), _compId(compId), _collisionGroup(cg), _position(position), _rotation(rotation), _scale(scale), _transform(rotation.toBullet(), position.toBullet()), _rigidBody(), _oldScale(1.0f, 1.0f, 1.0f), _linearVelocity(), _centralForce(), _shatterInterest(shatterInterest), _parentShape(nullptr), _childShapes() {
	...
	bool PhysicsNode::addChild(const int64_t compId, const Vec3 & position, const Quaternion & rotation, const Vec3 & scale, const api::CollisionGroup & cg, const api::PhysicalStateComponent::ShapeType shapeType, const attributeMap & shapeParams) {
		...
		if (_childShapes.empty()) {
			Vec3 angularFactor(0.5, 0.5, 0.5);
			Vec3 linearFactor(1.0, 1.0, 1.0);
			double mass = 0.0;
			double restitution = 0.0;
			double friction = 0.0;
			int activationState = DISABLE_DEACTIVATION;
			double linearDamping = 0.0;
			double angularDamping = 0.5;
			...
			if (_parentShape != nullptr) {
				btRigidBody::btRigidBodyConstructionInfo groundRigidBodyCI(mass, this, _parentShape, btVector3(0.0, 0.0, 0.0));
				_rigidBody = new btRigidBody(groundRigidBodyCI);
			} else {
				btRigidBody::btRigidBodyConstructionInfo groundRigidBodyCI(mass, this, newShape, btVector3(0.0, 0.0, 0.0));
				_rigidBody = new btRigidBody(groundRigidBodyCI);
			}
