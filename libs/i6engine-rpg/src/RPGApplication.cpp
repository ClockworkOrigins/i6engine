...
namespace rpg {
	...
	void RPGApplication::loadLevel(const std::string & level) const {
		...
		std::vector<std::string> vec = utils::split(level, "/");
		std::string levelStartup = utils::split(vec.back(), ".").front();
		api::EngineController::GetSingletonPtr()->getScriptingFacade()->callFunctionWithCallback<void>("Startup_" + levelStartup, []() {});
#endif
	}
