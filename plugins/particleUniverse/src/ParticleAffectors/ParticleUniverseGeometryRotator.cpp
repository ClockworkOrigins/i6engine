...
namespace ParticleUniverse {
	...
	const Real GeometryRotator::DEFAULT_ROTATION_SPEED = 10.0;
	const Vector3 GeometryRotator::DEFAULT_ROTATION_AXIS(0, 0, 0);

	GeometryRotator::GeometryRotator() : ParticleAffector(), mScaledRotationSpeed(0.0), mUseOwnRotationSpeed(DEFAULT_USE_OWN), mQ(Quaternion::IDENTITY), mRotationAxis(DEFAULT_ROTATION_AXIS), mRotationAxisSet(false) {
	...
	void GeometryRotator::resetRotationAxis() {
		mDynRotationSpeed = PU_NEW_T(DynamicAttributeFixed, MEMCATEGORY_SCENE_OBJECTS)();
		static_cast<DynamicAttributeFixed *>(mDynRotationSpeed)->setValue(DEFAULT_ROTATION_SPEED);
		mRotationAxisSet = false;
	}
	
	DynamicAttribute * GeometryRotator::getRotationSpeed() const {
	...
	bool GeometryRotator::useOwnRotationSpeed() const {
