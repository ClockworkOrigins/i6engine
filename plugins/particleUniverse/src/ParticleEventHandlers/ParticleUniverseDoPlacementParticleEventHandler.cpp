...
namespace ParticleUniverse {
	...
	DoPlacementParticleEventHandler::DoPlacementParticleEventHandler() : ParticleEventHandler(), TechniqueListener(), mForceEmitterName(BLANK_STRING), mNumberOfParticles(DEFAULT_NUMBER_OF_PARTICLES), mTechnique(nullptr), mEmitter(nullptr), mFound(false), mAlwaysUsePosition(true), mBaseParticle(nullptr), mInheritPosition(true), mInheritDirection(false), mInheritOrientation(false), mInheritTimeToLive(false), mInheritMass(false), mInheritTextureCoordinate(false), mInheritColour(false), mInheritParticleWidth(false), mInheritParticleHeight(false), mInheritParticleDepth(false) {
	}
	
	DoPlacementParticleEventHandler::~DoPlacementParticleEventHandler() {
	...
	ParticleEmitter * DoPlacementParticleEventHandler::getForceEmitter() const {
		return mEmitter;
	}
	
	void DoPlacementParticleEventHandler::removeAsListener() {
