...
namespace ParticleUniverse {
	...
	void DoPlacementParticleEventHandlerWriter::write(ParticleScriptSerializer * serializer, const IElement * element) {
		...
		if (eventHandler->getForceEmitterName() != BLANK_STRING) {
			serializer->writeLine(token[TOKEN_DOPLACE_FORCE_EMITTER], eventHandler->getForceEmitterName(), 16);
		}
		if (eventHandler->getNumberOfParticles() != DoPlacementParticleEventHandler::DEFAULT_NUMBER_OF_PARTICLES) {
			serializer->writeLine(token[TOKEN_DOPLACE_NUMBER_OF_PARTICLES], StringConverter::toString(eventHandler->getNumberOfParticles()), 16);
		}
		if ((!eventHandler->isInheritPosition())) {
			serializer->writeLine(token[TOKEN_INHERIT_POSITION], StringConverter::toString((!eventHandler->isInheritPosition())), 16);
		}
		if (eventHandler->isInheritDirection()) {
			serializer->writeLine(token[TOKEN_INHERIT_DIRECTION], StringConverter::toString(eventHandler->isInheritDirection()), 16);
		}
		if (eventHandler->isInheritOrientation()) {
			serializer->writeLine(token[TOKEN_INHERIT_ORIENTATION], StringConverter::toString(eventHandler->isInheritOrientation()), 16);
		}
		if (eventHandler->isInheritTimeToLive()) {
			serializer->writeLine(token[TOKEN_INHERIT_TIME_TO_LIVE], StringConverter::toString(eventHandler->isInheritTimeToLive()), 16);
		}
		if (eventHandler->isInheritMass()) {
			serializer->writeLine(token[TOKEN_INHERIT_MASS], StringConverter::toString(eventHandler->isInheritMass()), 16);
		}
		if (eventHandler->isInheritTextureCoordinate()) {
			serializer->writeLine(token[TOKEN_INHERIT_TEXTURE_COORDINATE], StringConverter::toString(eventHandler->isInheritTextureCoordinate()), 16);
		}
		if (eventHandler->isInheritColour()) {
			serializer->writeLine(token[TOKEN_INHERIT_COLOUR], StringConverter::toString(eventHandler->isInheritColour()), 16);
		}
		if (eventHandler->isInheritParticleWidth()) {
			serializer->writeLine(token[TOKEN_INHERIT_WIDTH], StringConverter::toString(eventHandler->isInheritParticleWidth()), 16);
		}
		if (eventHandler->isInheritParticleHeight()) {
			serializer->writeLine(token[TOKEN_INHERIT_HEIGHT], StringConverter::toString(eventHandler->isInheritParticleHeight()), 16);
		}
		if (eventHandler->isInheritParticleDepth()) {
			serializer->writeLine(token[TOKEN_INHERIT_DEPTH], StringConverter::toString(eventHandler->isInheritParticleDepth()), 16);
		}
