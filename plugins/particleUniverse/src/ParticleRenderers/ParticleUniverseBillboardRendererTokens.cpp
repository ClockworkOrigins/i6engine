...
namespace ParticleUniverse {
	...
	void BillboardRendererWriter::write(ParticleScriptSerializer * serializer, const IElement * element) {
		...
		if (renderer->getBillboardType() != BillboardRenderer::DEFAULT_BILLBOARD_TYPE) {
			...
			if (renderer->getBillboardType() == BillboardRenderer::BBT_ORIENTED_COMMON) {
				billboardType = token[TOKEN_BILLBOARD_ORIENTED_COMMON];
			} else if (renderer->getBillboardType() == BillboardRenderer::BBT_ORIENTED_SELF) {
				billboardType = token[TOKEN_BILLBOARD_ORIENTED_SELF];
			} else if (renderer->getBillboardType() == BillboardRenderer::BBT_ORIENTED_SHAPE) {
				billboardType = token[TOKEN_BILLBOARD_ORIENTED_SHAPE];
			} else if (renderer->getBillboardType() == BillboardRenderer::BBT_PERPENDICULAR_COMMON) {
				billboardType = token[TOKEN_BILLBOARD_PERPENDICULAR_COMMON];
			} else if (renderer->getBillboardType() == BillboardRenderer::BBT_PERPENDICULAR_SELF) {
				billboardType = token[TOKEN_BILLBOARD_PERPENDICULAR_SELF];
			}
		...
		if (renderer->getBillboardOrigin() != BillboardRenderer::DEFAULT_ORIGIN) {
			...
			if (renderer->getBillboardOrigin() == Ogre::BBO_BOTTOM_CENTER) {
				origin = token[TOKEN_BILLBOARD_BOTTOM_CENTER];
			} else if (renderer->getBillboardOrigin() == Ogre::BBO_BOTTOM_LEFT) {
				origin = token[TOKEN_BILLBOARD_BOTTON_LEFT];
			} else if (renderer->getBillboardOrigin() == Ogre::BBO_BOTTOM_RIGHT) {
				origin = token[TOKEN_BILLBOARD_BOTTOM_RIGHT];
			} else if (renderer->getBillboardOrigin() == Ogre::BBO_CENTER_LEFT) {
				origin = token[TOKEN_BILLBOARD_CENTER_LEFT];
			} else if (renderer->getBillboardOrigin() == Ogre::BBO_CENTER_RIGHT) {
				origin = token[TOKEN_BILLBOARD_CENTER_RIGHT];
			} else if (renderer->getBillboardOrigin() == Ogre::BBO_TOP_CENTER) {
				origin = token[TOKEN_BILLBOARD_TOP_CENTER];
			} else if (renderer->getBillboardOrigin() == Ogre::BBO_TOP_LEFT) {
				origin = token[TOKEN_BILLBOARD_TOP_LEFT];
			} else if (renderer->getBillboardOrigin() == Ogre::BBO_TOP_RIGHT) {
				origin = token[TOKEN_BILLBOARD_TOP_RIGHT];
			}
		...
		if (renderer->getBillboardRotationType() != BillboardRenderer::DEFAULT_ROTATION_TYPE) {
			...
			if (renderer->getBillboardRotationType() == Ogre::BBR_TEXCOORD) {
				rotationType = token[TOKEN_BILLBOARD_TEXCOORD];
			}
			serializer->writeLine(token[TOKEN_BILLBOARD_ROTATION_TYPE], rotationType, 12);
		}

		if (renderer->getCommonDirection() != BillboardRenderer::DEFAULT_COMMON_DIRECTION) {
			serializer->writeLine(token[TOKEN_BILLBOARD_COMMON_DIRECTION], StringConverter::toString(renderer->getCommonDirection()), 12);
		}
		if (renderer->getCommonUpVector() != BillboardRenderer::DEFAULT_COMMON_UP_VECTOR) {
			serializer->writeLine(token[TOKEN_BILLBOARD_COMMON_UP_VECTOR], StringConverter::toString(renderer->getCommonUpVector()), 12);
		}
		if (renderer->isPointRenderingEnabled() != BillboardRenderer::DEFAULT_POINT_RENDERING) {
			serializer->writeLine(token[TOKEN_BILLBOARD_POINT_RENDERING], StringConverter::toString(renderer->isPointRenderingEnabled()), 12);
		}
		if (renderer->isUseAccurateFacing() != BillboardRenderer::DEFAULT_ACCURATE_FACING) {
			serializer->writeLine(token[TOKEN_BILLBOARD_ACCURATE_FACING], StringConverter::toString(renderer->isUseAccurateFacing()), 12);
		}
