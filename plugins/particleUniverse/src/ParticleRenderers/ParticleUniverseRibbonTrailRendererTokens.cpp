...
namespace ParticleUniverse {
	...
	void RibbonTrailRendererWriter::write(ParticleScriptSerializer * serializer, const IElement * element) {
		...
		if (renderer->isUseVertexColours() != RibbonTrailRenderer::DEFAULT_USE_VERTEX_COLOURS) {
			serializer->writeLine(token[TOKEN_USE_VERTEX_COLOURS], StringConverter::toString(renderer->isUseVertexColours()), 12);
		}
		if (renderer->getMaxChainElements() != RibbonTrailRenderer::DEFAULT_MAX_ELEMENTS) {
			serializer->writeLine(token[TOKEN_MAX_ELEMENTS], StringConverter::toString(renderer->getMaxChainElements()), 12);
		}
		if (!almostEquals(renderer->getTrailLength(), RibbonTrailRenderer::DEFAULT_LENGTH)) {
			serializer->writeLine(token[TOKEN_RIBBONTRAIL_LENGTH], StringConverter::toString(renderer->getTrailLength()), 12);
		}
		if (!almostEquals(renderer->getTrailWidth(), RibbonTrailRenderer::DEFAULT_WIDTH)) {
			serializer->writeLine(token[TOKEN_RIBBONTRAIL_WIDTH], StringConverter::toString(renderer->getTrailWidth()), 12);
		}
		if (renderer->isRandomInitialColour() != RibbonTrailRenderer::DEFAULT_RANDOM_INITIAL_COLOUR) {
			serializer->writeLine(token[TOKEN_RANDOM_INITIAL_COLOUR], StringConverter::toString(renderer->isRandomInitialColour()), 12);
		}
		if (renderer->getInitialColour() != RibbonTrailRenderer::DEFAULT_INITIAL_COLOUR) {
			serializer->writeLine(token[TOKEN_INITIAL_COLOUR], StringConverter::toString(renderer->getInitialColour()), 12);
		}
		if (renderer->getColourChange() != RibbonTrailRenderer::DEFAULT_COLOUR_CHANGE) {
			serializer->writeLine(token[TOKEN_COLOUR_CHANGE], StringConverter::toString(renderer->getColourChange()), 12);
		}
