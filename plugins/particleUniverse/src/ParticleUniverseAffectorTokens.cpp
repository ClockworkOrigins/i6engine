...
namespace ParticleUniverse {
	 ...
	 * AffectorTranslator
	 *************************************************************************/
	AffectorTranslator::AffectorTranslator() : mAffector(nullptr) {
	}
					...
					if (passValidatePropertyNoValues(compiler, prop, token[TOKEN_AFFECTOR_EXCLUDE_EMITTER])) {
						for (AbstractNodeList::iterator j = prop->values.begin(); j != prop->values.end(); ++j) {
							...
							if (getString(*j, &val)) {
								...
								compiler->addError(ScriptCompiler::CE_NUMBEREXPECTED, prop->file, int(prop->line), "PU Compiler: Error in exclude_emitter arguments");
							}
						}
					}
	...
	void ParticleAffectorWriter::write(ParticleScriptSerializer * serializer, const IElement * element) {
		...
		if (affector->_getOriginalEnabled() != ParticleAffector::DEFAULT_ENABLED) {
			serializer->writeLine(token[TOKEN_ENABLED], StringConverter::toString(affector->_getOriginalEnabled()), 12);
		}
		if (affector->position != ParticleAffector::DEFAULT_POSITION) {
			serializer->writeLine(token[TOKEN_POSITION], StringConverter::toString(affector->position), 12);
		}
		if (!almostEquals(affector->mass, ParticleAffector::DEFAULT_MASS)) {
			serializer->writeLine(token[TOKEN_AFFECTOR_MASS], StringConverter::toString(affector->mass), 12);
		}
		...
		if (affector->getAffectSpecialisation() != ParticleAffector::DEFAULT_SPECIALISATION) {
			serializer->writeLine(token[TOKEN_AFFECTOR_SPECIALISATION], affectSpecialisation, 12);
		}

		const list<String> excludedEmitters = affector->getEmittersToExclude();
		String emitterList = BLANK_STRING;
		if (!excludedEmitters.empty()) {
			for (list<String>::const_iterator it = excludedEmitters.begin(); it != excludedEmitters.end(); ++it) {
