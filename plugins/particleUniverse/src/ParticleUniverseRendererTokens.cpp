...
namespace ParticleUniverse {
	 ...
	 * RendererTranslator
	 *************************************************************************/
	RendererTranslator::RendererTranslator() : mRenderer(nullptr) {
	}
	...
	void ParticleRendererWriter::write(ParticleScriptSerializer * serializer, const IElement * element) {
		...
		if (renderer->getRenderQueueGroup() != ParticleRenderer::DEFAULT_RENDER_QUEUE_GROUP) {
			serializer->writeLine(token[TOKEN_RENDERER_Q_GROUP], StringConverter::toString(renderer->getRenderQueueGroup()), 12);
		}
		if (renderer->isSorted() != ParticleRenderer::DEFAULT_SORTED) {
			serializer->writeLine(token[TOKEN_RENDERER_SORTING], StringConverter::toString(renderer->isSorted()), 12);
		}
		if (renderer->getTextureCoordsRows() != ParticleRenderer::DEFAULT_TEXTURECOORDS_ROWS) {
			serializer->writeLine(token[TOKEN_RENDERER_TEXCOORDS_ROWS], StringConverter::toString(renderer->getTextureCoordsRows()), 12);
		}
		if (renderer->getTextureCoordsColumns() != ParticleRenderer::DEFAULT_TEXTURECOORDS_COLUMNS) {
			serializer->writeLine(token[TOKEN_RENDERER_TEXCOORDS_COLUMNS], StringConverter::toString(renderer->getTextureCoordsColumns()), 12);
		}
		...
		if (!uvList.empty()) {
			...
			for (vector<Ogre::FloatRect *>::const_iterator it = uvList.begin(); it != uvList.end(); ++it) {
				serializer->writeLine(token[TOKEN_RENDERER_TEXCOORDS_DEFINE], StringConverter::toString(*it), 12);
			}
			serializer->writeLine("}", 12);
		}

		if (renderer->getUseSoftParticles() != ParticleRenderer::DEFAULT_USE_SOFT_PARTICLES) {
			serializer->writeLine(token[TOKEN_RENDERER_USE_SOFT_PARTICLES], StringConverter::toString(renderer->getUseSoftParticles()), 12);
		}
		if (!almostEquals(renderer->getSoftParticlesContrastPower(), ParticleRenderer::DEFAULT_SOFT_PARTICLES_CONTRAST_POWER)) {
			serializer->writeLine(token[TOKEN_RENDERER_SOFT_PARTICLES_CONTRAST_POWER], StringConverter::toString(renderer->getSoftParticlesContrastPower()), 12);
		}
		if (!almostEquals(renderer->getSoftParticlesScale(), ParticleRenderer::DEFAULT_SOFT_PARTICLES_SCALE)) {
			serializer->writeLine(token[TOKEN_RENDERER_SOFT_PARTICLES_SCALE], StringConverter::toString(renderer->getSoftParticlesScale()), 12);
		}
		if (!almostEquals(renderer->getSoftParticlesDelta(), ParticleRenderer::DEFAULT_SOFT_PARTICLES_DELTA)) {
			serializer->writeLine(token[TOKEN_RENDERER_SOFT_PARTICLES_DELTA], StringConverter::toString(renderer->getSoftParticlesDelta()), 12);
		}
	}
