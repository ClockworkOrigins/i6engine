...
namespace ParticleUniverse {
	...
	ParticleSystemManager::ParticleSystemManager() : mAutoLoadMaterials(true), mDepthTextureName(BLANK_STRING), mDepthMaterialName(BLANK_STRING), mDepthVertexName(BLANK_STRING), mDepthFragmentName(BLANK_STRING), mDepthMap(nullptr), mDepthTechnique(nullptr), mDepthPass(nullptr), mDepthMapExtern(false), mDepthScale(1.0), mLastCreatedParticleSystemTemplateName(BLANK_STRING) {
	...
	void ParticleSystemManager::destroyAllParticleSystemTemplates() { 
		...
		for (ParticleSystemTemplateMap::iterator t = mParticleSystemTemplates.begin(); t != mParticleSystemTemplates.end(); ++t) {
			PU_DELETE t->second;
		}
		mParticleSystemTemplates.clear();
	}
	
	BoxSet * ParticleSystemManager::createBoxSet(const String & name, Ogre::SceneManager * sceneManager, const uint poolSize) {
		Ogre::NameValuePairList params;
		params["poolSize"] = StringConverter::toString(poolSize);
	
		return static_cast<BoxSet *>(sceneManager->createMovableObject(name, BoxSetFactory::PU_FACTORY_TYPE_NAME, &params));
	}
	...
	SphereSet * ParticleSystemManager::createSphereSet(const String & name, Ogre::SceneManager * sceneManager, const uint poolSize) {
		Ogre::NameValuePairList params;
		params["poolSize"] = StringConverter::toString(poolSize);
	
		return static_cast<SphereSet *>(sceneManager->createMovableObject(name, SphereSetFactory::PU_FACTORY_TYPE_NAME, &params));
	}
	...
	void ParticleSystemManager::removeEmitterFactory(const String & emitterType) {
		EmitterFactoryMap::iterator it = mEmitterFactories.find(emitterType);
		if (it == mEmitterFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find emitter factory to remove.", "ParticleSystemManager::removeEmitterFactory");
		}
	...
	void ParticleSystemManager::destroyEmitterFactory(const String & emitterType) {
		EmitterFactoryMap::iterator it = mEmitterFactories.find(emitterType);
		if (it == mEmitterFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find emitter factory to destroy.", "ParticleSystemManager::destroyEmitterFactory");
		}
	...
	ParticleEmitter * ParticleSystemManager::createEmitter(const String & emitterType) {
		...
		EmitterFactoryMap::iterator it = mEmitterFactories.find(emitterType);
		if (it == mEmitterFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested emitter type.", "ParticleSystemManager::createEmitter");
		}
	...
	void ParticleSystemManager::destroyEmitter(ParticleEmitter * emitter) {
		...
		if (it == mEmitterFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested emitter type.", "ParticleSystemManager::destroyEmitter");
		}
	...
	void ParticleSystemManager::removeAffectorFactory(const String & affectorType) {
		AffectorFactoryMap::iterator it = mAffectorFactories.find(affectorType);
		if (it == mAffectorFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find affector factory to remove.", "ParticleSystemManager::removeAffectorFactory");
		}
	...
	void ParticleSystemManager::destroyAffectorFactory(const String & affectorType) {
		AffectorFactoryMap::iterator it = mAffectorFactories.find(affectorType);
		if (it == mAffectorFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find affector factory to destroy.", "ParticleSystemManager::destroyAffectorFactory");
		}
	...
	ParticleAffector * ParticleSystemManager::createAffector(const String & affectorType) {
		...
		if (it == mAffectorFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested affector type.", "ParticleSystemManager::createAffector");
		}
	...
	void ParticleSystemManager::destroyAffector(ParticleAffector * affector) {
		...
		if (it == mAffectorFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested affector type.", "ParticleSystemManager::destroyAffector");
		}
	...
	void ParticleSystemManager::removeObserverFactory(const String & observerType) {
		ObserverFactoryMap::iterator it = mObserverFactories.find(observerType);
		if (it == mObserverFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find observer factory to remove.", "ParticleSystemManager::removeObserverFactory");
		}
	...
	void ParticleSystemManager::destroyObserverFactory(const String & observerType) {
		ObserverFactoryMap::iterator it = mObserverFactories.find(observerType);
		if (it == mObserverFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find observer factory to destroy.", "ParticleSystemManager::destroyObserverFactory");
		}
	...
	ParticleObserver * ParticleSystemManager::createObserver(const String & observerType) {
		...
		if (it == mObserverFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested observer type.", "ParticleSystemManager::createObserver");
		}
	...
	void ParticleSystemManager::destroyObserver(ParticleObserver * observer) {
		...
		if (it == mObserverFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested observer type.", "ParticleSystemManager::destroyObserver");
		}
	...
	void ParticleSystemManager::removeEventHandlerFactory(const String & eventHandlerType) {
		EventHandlerFactoryMap::iterator it = mEventHandlerFactories.find(eventHandlerType);
		if (it == mEventHandlerFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find event handler factory to remove.", "ParticleSystemManager::removeEventHandlerFactory");
		}
	...
	void ParticleSystemManager::destroyEventHandlerFactory(const String & eventHandlerType) {
		EventHandlerFactoryMap::iterator it = mEventHandlerFactories.find(eventHandlerType);
		if (it == mEventHandlerFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find event handler factory to destroy.", "ParticleSystemManager::destroyEventHandlerFactory");
		}
	...
	ParticleEventHandler * ParticleSystemManager::createEventHandler(const String & eventHandlerType) {
		...
		if (it == mEventHandlerFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested event handler type.", "ParticleSystemManager::createEventHandler");
		}
	...
	void ParticleSystemManager::destroyEventHandler(ParticleEventHandler * eventHandler) {
		...
		if (it == mEventHandlerFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested event handler type.", "ParticleSystemManager::destroyEventHandler");
		}
	...
	void ParticleSystemManager::removeRendererFactory(const String & rendererType) {
		RendererFactoryMap::iterator it = mRendererFactories.find(rendererType);
		if (it == mRendererFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find renderer factory to remove.", "ParticleSystemManager::removeRendererFactory");
		}
	...
	void ParticleSystemManager::destroyRendererFactory(const String & rendererType) {
		RendererFactoryMap::iterator it = mRendererFactories.find(rendererType);
		if (it == mRendererFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find renderer factory to destroy.", "ParticleSystemManager::destroyRendererFactory");
		}
	...
	ParticleRenderer * ParticleSystemManager::createRenderer(const String & rendererType) {
		...
		if (it == mRendererFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested renderer type.", "ParticleSystemManager::createRenderer");
		}
	...
	void ParticleSystemManager::destroyRenderer(ParticleRenderer * renderer) {
		...
		if (it == mRendererFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested renderer type.", "ParticleSystemManager::destroyRenderer");
		}
	...
	void ParticleSystemManager::removeExternFactory(const String & externType) {
		ExternFactoryMap::iterator it = mExternFactories.find(externType);
		if (it == mExternFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find extern factory to remove.", "ParticleSystemManager::removeExternFactory");
		}
	...
	void ParticleSystemManager::destroyExternFactory(const String & externType) {
		ExternFactoryMap::iterator it = mExternFactories.find(externType);
		if (it == mExternFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find extern factory to destroy.", "ParticleSystemManager::destroyExternFactory");
		}
	...
	Extern * ParticleSystemManager::createExtern(const String & externType) {
		...
		if (it == mExternFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested extern type.", "ParticleSystemManager::createExtern");
		}
	...
	void ParticleSystemManager::destroyExtern(Extern * externObject) {
		...
		if (it == mExternFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested extern type.", "ParticleSystemManager::destroyExtern");
		}
	...
	void ParticleSystemManager::removeBehaviourFactory(const String & behaviourType) {
		BehaviourFactoryMap::iterator it = mBehaviourFactories.find(behaviourType);
		if (it == mBehaviourFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find behaviour factory to remove.", "ParticleSystemManager::removeBehaviourFactory");
		}
	...
	void ParticleSystemManager::destroyBehaviourFactory(const String & behaviourType) {
		BehaviourFactoryMap::iterator it = mBehaviourFactories.find(behaviourType);
		if (it == mBehaviourFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find behaviour factory to destroy.", "ParticleSystemManager::destroyBehaviourFactory");
		}
	...
	ParticleBehaviour * ParticleSystemManager::createBehaviour(const String & behaviourType) {
		...
		if (it == mBehaviourFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested behaviour type.", "ParticleSystemManager::createBehaviour");
		}
	...
	void ParticleSystemManager::destroyBehaviour(ParticleBehaviour * behaviour) {
		...
		if (it == mBehaviourFactories.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find requested behaviour type.", "ParticleSystemManager::destroyBehaviour");
		}
	...
	const String & ParticleSystemManager::getLastCreatedParticleSystemTemplateName() {
	...
	void ParticleSystemManager::addParticleSystemTemplate(const String & name, ParticleSystem * systemTemplate) {
		...
		if (mParticleSystemTemplates.find(name) != mParticleSystemTemplates.end()) {
			EXCEPT(Exception::ERR_DUPLICATE_ITEM, "PU: ParticleSystem template with name '" + name + "' already exists.", "ParticleSystemManager::addParticleSystemTemplate");
		}
	...
	void ParticleSystemManager::particleSystemTemplateNames(vector<String> & v) {
		for (ParticleSystemTemplateMap::iterator it = mParticleSystemTemplates.begin(); it != mParticleSystemTemplates.end(); ++it) {
			v.push_back((*it).first);
		}
	}
	
	ParticleSystem * ParticleSystemManager::createParticleSystem(const String & name, const String & templateName, Ogre::SceneManager * sceneManager) {
		// Validate name
		if (mParticleSystems.find(name) != mParticleSystems.end()) {
			EXCEPT(Exception::ERR_DUPLICATE_ITEM, "PU: ParticleSystem with name '" + name + "' already exists.", "ParticleSystemManager::createParticleSystem");
		}

		Ogre::NameValuePairList params;
		params["templateName"] = templateName;
	
		ParticleSystem * system = static_cast<ParticleSystem *>(sceneManager->createMovableObject(name, ParticleSystemFactory::PU_FACTORY_TYPE_NAME, &params));
	...
	ParticleSystem * ParticleSystemManager::createParticleSystem(const String & name, Ogre::SceneManager * sceneManager) {
		// Validate name
		if (mParticleSystems.find(name) != mParticleSystems.end()) {
			EXCEPT(Exception::ERR_DUPLICATE_ITEM, "PU: ParticleSystem with name '" + name + "' already exists.", "ParticleSystemManager::createParticleSystem");
		}
	...
	void ParticleSystemManager::destroyParticleSystem(const String & particleSystemName, Ogre::SceneManager * sceneManager) {
		// Remove it from the map although it doesnŽt exist anymore
	...
	void ParticleSystemManager::destroyAllParticleSystems(Ogre::SceneManager * sceneManager) {
		...
		// Delete all Particle Systems in the specified sceneManager. This doesnŽt include the templates
	...
	IAlias * ParticleSystemManager::getAlias(const String & aliasName) {
		...
		if (it == mAliasMap.end()) {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Cannot find alias.", "ParticleSystemManager::getAlias");
		}
	...
	void ParticleSystemManager::destroyAlias(IAlias * alias) {
		...
		case IAlias::AT_UNDEFINED: {
			EXCEPT(Exception::ERR_INVALIDPARAMS, "PU: Trying to delete alias of undetermined type.", "ParticleSystemManager::~ParticleSystemManager");
			break;
		}
		default: {
			break;
	...
	void ParticleSystemManager::destroyAllAliasses() {
	...
	ParticleSystemManager::AliasMap * ParticleSystemManager::_getAliasMap() {
		return &mAliasMap;
	}
	...
	ParticleScriptSerializer * ParticleSystemManager::getParticleScriptSerializer() const {
		return mParticleScriptSerializer;
	}
	
	CameraDependency * ParticleSystemManager::createCameraDependency() {
	...
	void ParticleSystemManager::createDepthMap(Camera * camera, Ogre::SceneManager * sceneManager) {
		...
		if (mDepthTextureName.empty()) {
			...
			Ogre::TexturePtr depthTexturePtr = Ogre::TextureManager::getSingleton().createManual(mDepthTextureName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, Ogre::TEX_TYPE_2D, uint(camera->getViewport()->getActualWidth()), uint(camera->getViewport()->getActualHeight()), 0, Ogre::PF_R8G8B8A8, Ogre::TU_RENDERTARGET);
		}
		...
		if (mDepthMaterialName.empty()) {
			...
			Ogre::HighLevelGpuProgramPtr vertexProgram = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(mDepthVertexName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "hlsl", Ogre::GPT_VERTEX_PROGRAM);	
			...
			Ogre::HighLevelGpuProgramPtr fragmentProgram = Ogre::HighLevelGpuProgramManager::getSingleton().createProgram(mDepthFragmentName, Ogre::ResourceGroupManager::DEFAULT_RESOURCE_GROUP_NAME, "hlsl", Ogre::GPT_FRAGMENT_PROGRAM);
...
//		}
	...
	void ParticleSystemManager::destroyDepthMap() {
		// Reset the depth map.
		if (mDepthMap && !mDepthMapExtern) {
	...
	void ParticleSystemManager::unregisterSoftParticlesRenderer(ParticleRenderer * renderer) {
		if (!mDepthMapExtern) {
			// Unregister the renderers and destroy the depthmap, because apparently it isnŽt used anymore
	...
	Real ParticleSystemManager::getDepthScale() const {
		return mDepthScale;
	}
	...
	const String & ParticleSystemManager::getDepthTextureName() {
	...
	void ParticleSystemManager::resetExternDepthTextureName() {
	...
	bool ParticleSystemManager::isAutoLoadMaterials() const {
	...
	void DepthMapTargetListener::preViewportUpdate(const Ogre::RenderTargetViewportEvent & evt) {
		...
		for (vector<ParticleRenderer *>::iterator it = mRenderers.begin(); it != mRenderers.end(); ++it) {
			(*it)->setVisible(false);
		}
	...
	bool DepthMapTargetListener::registeredRenderersEmpty() {
		return mRenderers.empty();
	}
	
	String ParticleSystemFactory::PU_FACTORY_TYPE_NAME = "PUParticleSystem";
	
	Ogre::MovableObject * ParticleSystemFactory::createInstanceImpl(const String & name, const Ogre::NameValuePairList * params) {
		if (params != nullptr) {
