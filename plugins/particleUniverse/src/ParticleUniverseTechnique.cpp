...
namespace ParticleUniverse {
	...
	const Real ParticleTechnique::DEFAULT_MAX_VELOCITY = 9999.0;
	
	ParticleTechnique::ParticleTechnique() : Particle(), IAlias(), IElement(), mParentSystem(nullptr), mName(BLANK_STRING), mVisualParticlePoolIncreased(false), mParticleEmitterPoolIncreased(false), mParticleTechniquePoolIncreased(false), mParticleAffectorPoolIncreased(false), mParticleSystemPoolIncreased(false), mVisualParticleQuota(DEFAULT_VISUAL_PARTICLE_QUOTA), mEmittedEmitterQuota(DEFAULT_EMITTED_EMITTER_QUOTA), mEmittedTechniqueQuota(DEFAULT_EMITTED_TECHNIQUE_QUOTA), mEmittedAffectorQuota(DEFAULT_EMITTED_AFFECTOR_QUOTA), mEmittedSystemQuota(DEFAULT_EMITTED_SYSTEM_QUOTA), mRenderer(nullptr), mSuppressNotifyEmissionChange(true), mMaterialName(BLANK_STRING), mDefaultWidth(DEFAULT_WIDTH), mDefaultHeight(DEFAULT_HEIGHT), mDefaultDepth(DEFAULT_DEPTH), mLodIndex(DEFAULT_LOD_INDEX), mCameraSquareDistance(0), mWidthCameraDependency(nullptr), mHeightCameraDependency(nullptr), mDepthCameraDependency(nullptr), mPrepareExtern(false), mPrepareBehaviour(false), mPrepareAffector(false), mPrepareEmitter(false), mWorldAABB(), mUpdateWorldAABB(false), mMinWorldExtend(Vector3::ZERO), mMaxWorldExtend(Vector3::ZERO), mHasExtents(false), mKeepLocal(DEFAULT_KEEP_LOCAL), _mParticleSystemScale(Vector3::UNIT_SCALE), _mParticleSystemScaleVelocity(1.0), mIsSpatialHashingUsed(false), mIsSpatialHashingInitialised(false), mSpatialHashingCellDimension(DEFAULT_SPATIAL_HASHING_CELL_DIM), mSpatialHashingCellOverlap(DEFAULT_SPATIAL_HASHING_CELL_OVERLAP), mSpatialHashTableSize(DEFAULT_SPATIAL_HASHING_TABLE_SIZE), mSpatialHashTableA(nullptr), mSpatialHashTableB(nullptr), mCurrentSpatialHashTable(nullptr), mSpatialHashingInterval(DEFAULT_SPATIAL_HASHING_INTERVAL), mSpatialHashingIntervalRemainder(0.0), mSpatialHashingIntervalSet(false), mSpatialHashingIntervalActivate(false), mMaxVelocity(DEFAULT_MAX_VELOCITY), mMaxVelocitySet(false), mMaxNumVisualParticles(0), mMaxNumEmitterParticles(0), mMaxNumTechniqueParticles(0), mMaxNumAffectorParticles(0), mMaxNumSystemParticles(0) {
	...
	void ParticleTechnique::_initParticleForEmission(Particle * particle) {
		if (!mTechniqueListenerList.empty()) {
			...
			for (TechniqueListenerIterator it = mTechniqueListenerList.begin(); it != mTechniqueListenerList.end(); ++it) {
	...
	void ParticleTechnique::_markForEmission() {
		...
		for (ParticleEmitterList::iterator emitterIt = mEmitters.begin(); emitterIt != mEmitters.end(); ++emitterIt) {
	...
	void ParticleTechnique::_resetMarkForEmission() {
		...
		for (ParticleAffectorIterator affectorIt = mAffectors.begin(); affectorIt != mAffectors.end(); ++affectorIt) {
	...
	void ParticleTechnique::_notifyRescaled(const Vector3 & scale) {
		...
		// Notify the emitters
		for (ParticleEmitterList::iterator itEmitter = mEmitters.begin(); itEmitter != mEmitters.end(); ++itEmitter) {
			(*itEmitter)->_notifyRescaled(scale);
		}
		
		// Notify the affectors
		for (ParticleAffectorIterator itAffector = mAffectors.begin(); itAffector != mAffectors.end(); ++itAffector) {
		...
		for (ParticleObserverIterator itObserver = mObservers.begin(); itObserver != mObservers.end(); ++itObserver) {
			(*itObserver)->_notifyRescaled(scale);
		}

		// Notify the behaviour
		for (ParticleBehaviourIterator itBehaviour = mBehaviours.begin(); itBehaviour != mBehaviours.end(); ++itBehaviour) {
			(*itBehaviour)->_notifyRescaled(scale);
		}

		// Notify the extern
		for (ExternIterator itExtern = mExterns.begin(); itExtern != mExterns.end(); ++itExtern) {
	...
	void ParticleTechnique::setDefaultWidth(const Real width) {
		mDefaultWidth = width;
		if (mRenderer) {
			mRenderer->_notifyDefaultDimensions(_mParticleSystemScale.x * mDefaultWidth, _mParticleSystemScale.y * mDefaultHeight, _mParticleSystemScale.z * mDefaultDepth);
		}
	}
	...
	void ParticleTechnique::removeEmitter(ParticleEmitter * emitter) {
		assert(emitter && "Emitter is null!");
		bool notify = false;
		for (ParticleEmitterList::iterator it = mEmitters.begin(); it != mEmitters.end(); ++it) {
			if (*it == emitter) {
		...
		emitter->setParentTechnique(nullptr);
	...
	ParticleEmitter * ParticleTechnique::getEmitter(const String & emitterName) const {
		...
		for (ParticleEmitterIterator it = mEmitters.begin(); it != mEmitters.end(); ++it) {
	...
	void ParticleTechnique::removeAffector(ParticleAffector * affector) {
		assert(affector && "Affector is null!");
		for (ParticleAffectorList::iterator it = mAffectors.begin(); it != mAffectors.end(); ++it) {
			if (*it == affector) {
				// Remove it
				mAffectors.erase(it);
				break;
		...
		// It has no parent anymore
		affector->setParentTechnique(nullptr);
	}
	...
	ParticleAffector * ParticleTechnique::getAffector(const String & affectorName) const {
		...
		for (ParticleAffectorIterator it = mAffectors.begin(); it != mAffectors.end(); ++it) {
	...
	void ParticleTechnique::removeObserver(ParticleObserver * observer) {
		assert(observer && "Observer is null!");
		for (ParticleObserverList::iterator it = mObservers.begin(); it != mObservers.end(); ++it) {
			if (*it == observer) {
				// Remove it
				mObservers.erase(it);
				break;
		...
		// It has no parent anymore
		observer->setParentTechnique(nullptr);
	}
	...
	ParticleObserver * ParticleTechnique::getObserver(const String & observerName) const {
		...
		for (ParticleObserverIterator it = mObservers.begin(); it != mObservers.end(); ++it) {
	...
	void ParticleTechnique::removeRenderer(ParticleRenderer * renderer) {
		...
		// It has no parent anymore
		renderer->setParentTechnique(nullptr);
	}
	...
	void ParticleTechnique::_removeBehaviourTemplate(ParticleBehaviour * behaviourTemplate) {
		assert(behaviourTemplate && "Behaviour is null!");
		for (ParticleBehaviourList::iterator it = mBehaviourTemplates.begin(); it != mBehaviourTemplates.end(); ++it) {
		...
		// It has no parent anymore
		behaviourTemplate->setParentTechnique(nullptr);
	}
	...
	ParticleBehaviour * ParticleTechnique::_getBehaviourTemplate(const String & behaviourType) const {
		...
		for (ParticleBehaviourIterator it = mBehaviourTemplates.begin(); it != mBehaviourTemplates.end(); ++it) {
	...
	void ParticleTechnique::removeExtern(Extern * externObject) {
		...
		for (ExternList::iterator it = mExterns.begin(); it != mExterns.end(); ++it) {
			if (*it == externObject) {
				// Remove it
				mExterns.erase(it);
				break;
		...
		// It has no parent anymore
		externObject->setParentTechnique(nullptr);
	}
	...
	Extern * ParticleTechnique::getExtern(const String & externName) const {
		...
		for (ExternIterator it = mExterns.begin(); it != mExterns.end(); ++it) {
	...
	void ParticleTechnique::_prepareTechnique() {
		...
		if (mIsSpatialHashingUsed && !mIsSpatialHashingInitialised) {
			...
			mSpatialHashTableA = PU_NEW_T(SpatialHashTable<Particle *>, MEMCATEGORY_SCENE_OBJECTS)(mSpatialHashTableSize, mSpatialHashingCellDimension, mSpatialHashingCellOverlap);

			if (mSpatialHashTableB) {
				PU_DELETE_T(mSpatialHashTableB, SpatialHashTable<Particle *>, MEMCATEGORY_SCENE_OBJECTS);
			}
			mSpatialHashTableB = PU_NEW_T(SpatialHashTable<Particle *>, MEMCATEGORY_SCENE_OBJECTS)(mSpatialHashTableSize, mSpatialHashingCellDimension, mSpatialHashingCellOverlap);
	...
	void ParticleTechnique::_prepareBehaviours() {
		/*
		@remarks
			Note, that the ParticleBehaviour template list stored in this ParticleTechnique is ŽpreparedŽ 
	...
	void ParticleTechnique::_notifyStart() {
		...
		if (!mEmitters.empty()) {
			for (ParticleEmitterIterator emitterIt = mEmitters.begin(); emitterIt != mEmitters.end(); ++emitterIt) {
	...
	void ParticleTechnique::_processDependencies() {
		...
		if (changed && mRenderer) {
			mRenderer->_notifyDefaultDimensions(_mParticleSystemScale.x * width, _mParticleSystemScale.y * height, _mParticleSystemScale.z * depth);
		}
	}
	...
	void ParticleTechnique::_processAffectors(Particle * particle, Real timeElapsed, bool firstParticle) {
		...
		/*  Iterate through all pooled affector objects (these are the affectors that are emitted).
			The pool itself isnŽt used, but an extraction of the pooled affectors.
		*/
		if (mCopyOfPooledAffectors.empty()) {
			return;
		}

		for (ParticleAffectorIterator pooledAffectorIt = mCopyOfPooledAffectors.begin(); pooledAffectorIt != mCopyOfPooledAffectors.end(); ++pooledAffectorIt) {
	...
	void ParticleTechnique::_initParticleForExpiration(Particle * particle, Real timeElapsed) {
		...
		/** Externs are also called to perform expiration activities. If needed, affectors and emitters may be added, but at the moment
			there is no reason for (and we donŽt want to waste cpu resources).
		*/
		if (!mExterns.empty()) {
			for (ExternIterator itExtern = mExterns.begin(); itExtern != mExterns.end(); ++itExtern) {
	...
	void ParticleTechnique::_emitParticles(Real timeElapsed) {
		...
		for (ParticleEmitterIterator itEmitter = mEmitters.begin(); itEmitter != mEmitters.end(); ++itEmitter) {
	...
	void ParticleTechnique::removeTechniqueListener(TechniqueListener * techniqueListener) {
		...
		for (TechniqueListenerList::iterator it = mTechniqueListenerList.begin(); it != mTechniqueListenerList.end(); ++it) {
			if (*it == techniqueListener) {
				// Remove it (donŽt destroy it, because the technique is not the owner)
	...
	void ParticleTechnique::logDebug() {
		...
		LogManager::getSingleton().logMessage("ParticleUniverse - Maximum number of visual particles emitted: " + StringConverter::toString(mMaxNumVisualParticles));

		if (mMaxNumEmitterParticles > 0) {
			LogManager::getSingleton().logMessage("ParticleUniverse - Maximum number of emitter particles emitted: " + StringConverter::toString(mMaxNumEmitterParticles));
		}

		if (mMaxNumTechniqueParticles > 0) {
			LogManager::getSingleton().logMessage("ParticleUniverse - Maximum number of technique particles emitted: " + StringConverter::toString(mMaxNumTechniqueParticles));
		}

		if (mMaxNumAffectorParticles > 0) {
			LogManager::getSingleton().logMessage("ParticleUniverse - Maximum number of affector particles emitted: " + StringConverter::toString(mMaxNumAffectorParticles));
		}

		if (mMaxNumSystemParticles > 0) {
			LogManager::getSingleton().logMessage("ParticleUniverse - Maximum number of particle systems emitted: " + StringConverter::toString(mMaxNumSystemParticles));
		}
	}
	...
	void ParticleTechnique::copyAttributesTo(ParticleTechnique * technique) {
		...
		// Copy all observers
		ParticleObserver * clonedObserver = nullptr;
		for (ParticleObserverIterator itObserver = mObservers.begin(); itObserver != mObservers.end(); ++itObserver) {
		...
		ParticleBehaviour * clonedBehaviour = nullptr;
		for (ParticleBehaviourIterator itBehaviour = mBehaviourTemplates.begin(); itBehaviour != mBehaviourTemplates.end(); ++itBehaviour) {
