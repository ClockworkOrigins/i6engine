...
namespace sample {
	...
	MultilanguageApplication::MultilanguageApplication() : i6engine::api::Application() {
	}
	...
	void MultilanguageApplication::AfterInitialize() {
		...
		gf->addPrint("ChangeLanguagePrint", "RPG/Blanko", 0.5, 0.48, i6engine::api::EngineController::GetSingleton().getTextManager()->getText("ChangeLanguageDescription_Key"), i6engine::api::gui::Alignment::Center, -1);
		gf->setSize("ChangeLanguagePrint", 1.0, 0.05);

		// this callback handles changing language
		i6engine::api::EngineController::GetSingleton().getLanguageManager()->addCallback([gf](const std::string &) {
			gf->setText("GermanButton", i6engine::api::EngineController::GetSingleton().getTextManager()->getText("German_Key"));
			gf->setText("EnglishButton", i6engine::api::EngineController::GetSingleton().getTextManager()->getText("English_Key"));
			gf->setText("ChangeLanguagePrint", i6engine::api::EngineController::GetSingleton().getTextManager()->getText("ChangeLanguageDescription_Key"));
		});
