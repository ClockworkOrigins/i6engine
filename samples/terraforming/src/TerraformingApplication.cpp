...
namespace sample {
	...
	TerraformingApplication::TerraformingApplication() : CommonApplication(true, false) {
	}
	...
	void TerraformingApplication::AfterInitialize() {
		...
		}
		{
			...
			i6engine::api::writeAttribute(params, "inputScale", 20.0); // scale factor for height in terrain
			...
			for (size_t i = 0; i < VERTICES; i++) {
				for (size_t j = 0; j < VERTICES; j++) {
					heightdata[i][j] = i6engine::utils::Random::GetSingleton().rand(101) / 100.0;
				}
			}
