...
PathFollowerPropertyWindow::PathFollowerPropertyWindow(wxWindow * parent, EditComponent * owner, const Ogre::String & name) : AffectorPropertyWindow(parent, owner, name) {
	_initProperties();
}

PathFollowerPropertyWindow::PathFollowerPropertyWindow(AffectorPropertyWindow * affectorPropertyWindow) : AffectorPropertyWindow(affectorPropertyWindow) {
	_initProperties();
}

PathFollowerPropertyWindow::PathFollowerPropertyWindow(wxWindow * parent, EditComponent * owner, ParticleUniverse::PathFollower * affector) : AffectorPropertyWindow(parent, owner, affector->getName()) {
...
void PathFollowerPropertyWindow::copyAttributeToAffector(wxPGProperty * prop, wxString propertyName) {
	...
	if (propertyName == PRNL_PATHFOLLOWER_POSITION || (propertyName.StartsWith(PRNL_POSITION) && propertyName.EndsWith(PRNL_POSITION_X)) || (propertyName.StartsWith(PRNL_POSITION) && propertyName.EndsWith(PRNL_POSITION_Y)) || (propertyName.StartsWith(PRNL_POSITION) && propertyName.EndsWith(PRNL_POSITION_Z))) {
		// Positions: List
...
void PathFollowerPropertyWindow::copyAttributesFromAffector(ParticleUniverse::ParticleAffector * affector) {
	...
	// Positions: List
	wxPGProperty * propTo = GetProperty(PRNL_PATHFOLLOWER_POSITION);
...
void PathFollowerPropertyWindow::_initProperties() {
