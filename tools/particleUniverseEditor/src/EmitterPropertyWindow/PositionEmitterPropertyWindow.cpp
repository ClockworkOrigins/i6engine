...
PositionEmitterPropertyWindow::PositionEmitterPropertyWindow(wxWindow * parent, EditComponent * owner, const Ogre::String & name) : EmitterPropertyWindow(parent, owner, name) {
	_initProperties();
}

PositionEmitterPropertyWindow::PositionEmitterPropertyWindow(EmitterPropertyWindow * emitterPropertyWindow) : EmitterPropertyWindow(emitterPropertyWindow) {
	_initProperties();
}

PositionEmitterPropertyWindow::PositionEmitterPropertyWindow(wxWindow * parent, EditComponent * owner, ParticleUniverse::PositionEmitter * emitter) : EmitterPropertyWindow(parent, owner, emitter->getName()) {
...
void PositionEmitterPropertyWindow::copyAttributeToEmitter(wxPGProperty * prop, wxString propertyName) {
	...
	if (propertyName == PRNL_POSITION_EMITTER_POSITION || (propertyName.StartsWith(PRNL_POSITION) && propertyName.EndsWith(PRNL_POSITION_X)) || (propertyName.StartsWith(PRNL_POSITION) && propertyName.EndsWith(PRNL_POSITION_Y)) || (propertyName.StartsWith(PRNL_POSITION) && propertyName.EndsWith(PRNL_POSITION_Z))) {
...
void PositionEmitterPropertyWindow::copyAttributesFromEmitter(ParticleUniverse::ParticleEmitter * emitter) {
	...
	for (ParticleUniverse::vector<Ogre::Vector3>::iterator it = positions.begin(); it != positions.end(); ++it) {
...
void PositionEmitterPropertyWindow::_initProperties() {
...
void PositionEmitterPropertyWindow::copyPositionsToPositionEmitter(wxPGProperty * prop, ParticleUniverse::PositionEmitter * positionEmitter) {
	...
	for (unsigned int i = 0; i < size; ++i) {
		Ogre::Vector3 vec = parentPropertyWithButtonAndPositions->getPosition(this, i, vec);
