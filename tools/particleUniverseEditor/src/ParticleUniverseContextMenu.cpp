...
ContextMenuElement::ContextMenuElement(ContextMenu * parent, wxWindowID id, const wxString & label, const wxString & bitmapNameEnabled, const wxString & bitmapNameDisabled, const wxPoint & pos, const wxSize & size, long style, const wxString & name) : wxStaticText(parent, id, label, pos, size, style, name), mEnabled(true), mParent(parent) {
...
wxStaticBitmap * ContextMenuElement::getEnabledIcon() {
...
ContextMenuCallbackObject::ContextMenuCallbackObject() {
}

ContextMenu::ContextMenu(wxWindow * parent, ContextMenuCallbackObject * callbackObject, const wxPoint & pos, const wxSize & size) : wxFrame(parent, wxID_ANY, wxT(""), pos, size, wxFRAME_NO_TASKBAR | wxFRAME_FLOAT_ON_PARENT | wxBORDER_DOUBLE | wxBORDER_RAISED), mParent(parent), mInitialised(false), mCallbackObject(callbackObject) {
}

void ContextMenu::addMenuElement(wxWindowID id, const wxString & label, const wxString & bitmapNameEnabled, const wxString & bitmapNameDisabled, const wxSize & size) {
	mMenuElements.push_back(new ContextMenuElement(this, id, label, bitmapNameEnabled, bitmapNameDisabled, wxDefaultPosition, size, wxALIGN_LEFT | wxST_NO_AUTORESIZE));
}

void ContextMenu::initialise() {
	if (mInitialised) {
		return;
	}
	...
	for (std::vector<ContextMenuElement *>::iterator it = mMenuElements.begin(); it != mMenuElements.end(); ++it) {
