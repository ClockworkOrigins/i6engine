...
ControlPointColourControl::ControlPointColourControl(std::vector<ControlPoint *> & v, wxWindow * parent, wxWindowID id, const wxPoint & pos, const wxSize & size, const wxString & name, const float min, const float max) : wxControl (parent, id, pos, size, 0, wxDefaultValidator, name), ContextMenuCallbackObject(), mParent(parent), mMovingControlPoint(nullptr), mMoving(false), mMin(min), mMax(max), mContextMenu(nullptr), mInitialControlPointList(v) {
	// Setup context menu
	mContextMenu = new ContextMenu(this, this, wxDefaultPosition, wxDefaultSize);
	mContextMenu->addMenuElement(ID_COLOUR_CP_CTRL, _("        Select colour"), wxT("change_colour.png"), wxT("change_colour.png"), wxSize(120, 16));
	mContextMenu->addMenuElement(ID_ALPHA_CP_CTRL, _("        Set alpha"), wxT("grid_icon.png"), wxT("grid_icon.png"), wxSize(120, 16));
...
ControlPointColourControl::~ControlPointColourControl() {
	delete mContextMenu;

	for (std::vector<ControlPoint *>::iterator it = mControlPointList.begin(); it != mControlPointList.end(); ++it) {
		delete *it;
	}
...
void ControlPointColourControl::OnMouseLButtonPressed(wxMouseEvent & event) {
	...
	mMovingControlPoint = nullptr;
	for (std::vector<ControlPoint *>::iterator it = mControlPointList.begin(); it != mControlPointList.end(); ++it) {
		wxPoint cpPos = (*it)->getPosition();
		if (event.GetPosition().x > cpPos.x && event.GetPosition().y > cpPos.y && event.GetPosition().x < cpPos.x + CONTROL_POINT_RECT_SIZE && event.GetPosition().y < cpPos.y + CONTROL_POINT_RECT_SIZE) {
	...
	for (std::vector<ControlPoint *>::iterator it = mControlPointList.begin(); it != mControlPointList.end(); ++it) {
		wxPoint cpPos = (*it)->getPosition();
		if (event.GetPosition().x > cpPos.x && event.GetPosition().y > cpPos.y + CONTROL_POINT_RECT_SIZE && event.GetPosition().x < cpPos.x + CONTROL_POINT_RECT_SIZE && event.GetPosition().y < cpPos.y + 2 * CONTROL_POINT_RECT_SIZE) {
	...
	if (event.GetPosition().x < CONTROL_POINT_RECT_SIZE || event.GetPosition().x > GetSize().x - CONTROL_POINT_RECT_SIZE - CONTROL_POINT_BORDER_SIZE_BOLD) {
		return;
	}

	// Create a new control point
	ControlPoint * cp = new ControlPoint(ControlPoint::CTRL_POINT_TYPE_SOLID, wxPoint(event.GetPosition().x, CONTROL_POINT_CONTROL_Y), true, GetSize().x);
...
void ControlPointColourControl::fillGradient(wxPaintDC & dc) {
	...
	sortControlPoints();
	std::vector<ControlPoint *>::iterator it = mControlPointList.begin();
...
void ControlPointColourControl::sortControlPoints() {
	std::multimap<int, ControlPoint *> cpMap;
	for (std::vector<ControlPoint *>::iterator it = mControlPointList.begin(); it != mControlPointList.end(); ++it) {
		cpMap.insert(std::make_pair((*it)->getPosition().x, *it));
	}
	mControlPointList.clear();
	for (std::multimap<int, ControlPoint *>::iterator itMap = cpMap.begin(); itMap != cpMap.end(); ++itMap) {
...
void ControlPointColourControl::callbackContextMenu(wxWindowID id, wxWindow * window) {
	...
	case ID_ALPHA_CP_CTRL: {
		if (mMovingControlPoint) {
			...
			wxPoint pos = GetParent()->GetParent()->GetPosition() + wxPoint(0.5 * (GetParent()->GetParent()->GetSize().x - sliderWindowWidth), 2 * CONTROL_POINT_CONTROL_RECT_HEIGHT); // Not really generic is it?
					
			SliderDialog dlg(this, wxID_ANY, _("Alpha"), pos, wxSize(sliderWindowWidth, 80), wxT("SliderDialog"), mMovingControlPoint->getColour().Alpha(), 0, 255, 255);
			if (dlg.ShowModal() == wxID_OK) {
				wxColour col(mMovingControlPoint->getColour().Red(), mMovingControlPoint->getColour().Green(), mMovingControlPoint->getColour().Blue(), dlg.getValue() * 255);
...
void ControlPointColourControl::reset() {
	for (std::vector<ControlPoint *>::iterator it = mControlPointList.begin(); it != mControlPointList.end(); ++it) {	
		delete *it;
	}
	mControlPointList.clear();
...
void ControlPointColourControl::addInitialControlpoints() {
	ControlPoint * cp = nullptr;
	for (std::vector<ControlPoint *>::iterator it = mInitialControlPointList.begin(); it != mInitialControlPointList.end(); ++it) {
		...
		if (it == --mInitialControlPointList.end()) {
			...
			cp = new ControlPoint(ControlPoint::CTRL_POINT_TYPE_SOLID, wxPoint(GetSize().x - CONTROL_POINT_RECT_SIZE - CONTROL_POINT_BORDER_SIZE_BOLD, y), false); // Must start at right
			cp->setColour((*it)->getColour());
			mControlPointList.push_back(cp);
		}
...
std::vector<ControlPoint *> & ControlPointColourControl::getControlPointList() {
