...
ControlPointControl::ControlPointControl(std::vector<ControlPoint *> & v, wxWindow * parent, wxWindowID id, const wxPoint & pos, const wxSize & size, const wxString & name) : wxControl (parent, id, pos, size, 0, wxDefaultValidator, name), mParent(parent), mMovingControlPoint(nullptr), mMoving(false), mInitialControlPointList(v), mMinXGraph(0.0f), mMaxXGraph(1.0f), mMinYGraph(0.0f), mMaxYGraph(1.0f), mNumberOfGridLines(20), mLinearInterpolation(true) {
...
ControlPointControl::~ControlPointControl() {
...
void ControlPointControl::OnPaint(wxPaintEvent & event) {
	...
	if (mControlPointList.size() > 1) {
		...
		if (mLinearInterpolation) {
			...
			std::vector<ControlPoint *>::iterator itNext = itFirst;
			++itNext;
			...
			for (int x = minX; x < maxX; ++x) {
				...
				dc.DrawLine(interpolatedStart.x + half, interpolatedStart.y + half, interpolatedEnd.x + half, interpolatedEnd.y + half);
				count += delta;
...
void ControlPointControl::OnMouseLButtonPressed(wxMouseEvent & event) {
	...
	if (event.GetPosition().y < 20 - 0.5 * CONTROL_POINT_RECT_SIZE || event.GetPosition().y > GetSize().y - 19 - 0.5 * CONTROL_POINT_RECT_SIZE || event.GetPosition().x < 36 - 0.5 * CONTROL_POINT_RECT_SIZE || event.GetPosition().x > GetSize().x - 22 - 0.5 * CONTROL_POINT_RECT_SIZE) {
		return;
	}

	// Create a new control point
	ControlPoint * cp = new ControlPoint(ControlPoint::CTRL_POINT_TYPE_GRID, event.GetPosition(), true);
...
void ControlPointControl::reset() {
...
void ControlPointControl::addInitialControlpoints() {
...
std::vector<ControlPoint*>& ControlPointControl::getControlPointList() {
...
float ControlPointControl::getMinXGraph() const {
	return mMinXGraph;
}

float ControlPointControl::getMinYGraph() const {
	return mMinYGraph;
}

float ControlPointControl::getMaxXGraph() const {
	return mMaxXGraph;
}

float ControlPointControl::getMaxYGraph() const {
	return mMaxYGraph;
}
...
void ControlPointControl::sortControlPoints() {
...
bool ControlPointControl::useLinearInterpolation() {
