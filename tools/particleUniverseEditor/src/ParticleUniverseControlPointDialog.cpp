...
HorizontalSlider::HorizontalSlider(ControlPointDialog * parent, wxWindowID id, int value, int minValue, int maxValue, const wxPoint & pos, const wxSize & size) : wxSlider (parent, id, value, minValue, maxValue, pos, size, wxSL_HORIZONTAL), mParent(parent) {
...
VerticalSlider::VerticalSlider(ControlPointDialog * parent, wxWindowID id, int value, int minValue, int maxValue, const wxPoint & pos, const wxSize & size) : wxSlider (parent, id, value, minValue, maxValue, pos, size, wxSL_VERTICAL | wxSL_INVERSE), mParent(parent) {
...
HorizontalScrollBar::HorizontalScrollBar(ControlPointDialog * parent, wxWindowID id, const wxPoint & pos, const wxSize & size) : wxScrollBar(parent, id, pos, size, wxHORIZONTAL), mParent(parent) {
...
VerticalScrollBar::VerticalScrollBar(ControlPointDialog * parent, wxWindowID id, const wxPoint & pos, const wxSize & size) : wxScrollBar(parent, id, pos, size, wxVERTICAL), mParent(parent) {
...
InterpolationRadioBox::InterpolationRadioBox(ControlPointDialog * parent, wxWindowID id, const wxArrayString & choices, int dim) : wxRadioBox(parent, id, _("Interpolation"), wxDefaultPosition, wxDefaultSize, choices, dim, wxRA_SPECIFY_COLS), mParent(parent) {
...
ControlPointDialog::ControlPointDialog(ParticleUniverse::InterpolationType interpolationType, const ParticleUniverse::DynamicAttributeCurved::ControlPointList controlPointList, wxWindow * parent, wxWindowID id, const wxString & title, const wxPoint & pos, const wxSize & size, const wxString & name) : wxDialog(parent, id, title, pos, size, wxDEFAULT_DIALOG_STYLE | wxSTAY_ON_TOP, name), mControlPointsForPUCommunication(controlPointList), mPreviousHorizontalScale(1), mPreviousVerticalScale(1), mPreviousHorizontalScroll(5), mPreviousVerticalScroll(5) {
	// Fill mControlPoints with values passed in the dialog
	ControlPoint * cp;
	for (ParticleUniverse::DynamicAttributeCurved::ControlPointList::const_iterator it = mControlPointsForPUCommunication.begin(); it != mControlPointsForPUCommunication.end(); ++it) {
...
bool ControlPointDialog::Destroy() {
	for (std::vector<ControlPoint *>::iterator it = mControlPoints.begin(); it != mControlPoints.end(); ++it) {
		delete (*it);
	}
...
const ParticleUniverse::DynamicAttributeCurved::ControlPointList ControlPointDialog::getControlPointList() {
...
ParticleUniverse::InterpolationType ControlPointDialog::getInterpolationType() {
