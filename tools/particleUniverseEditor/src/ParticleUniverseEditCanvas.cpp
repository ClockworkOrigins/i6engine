...
EditCanvas::EditCanvas(wxPanel* parent) : wxPanel(parent, wxID_ANY, wxPoint(0, 0), parent->GetSize(), 0, wxT("canvas")), mMouseConnector(0), mMousePosition(wxPoint(0, 0)), mMousePositionAdjustment(wxPoint(0, 0)), mSelectionMode(SM_SELECT_NONE), mStartPositionSelection(wxPoint(0, 0)), mEndPositionSelection(wxPoint(0, 0)) {
...
void EditCanvas::OnPaint(wxPaintEvent & event) {
	...
	for (std::vector<DrawConnector *>::iterator it = mDrawConnectors.begin(); it != mDrawConnectors.end(); ++it) {
		(*it)->drawLine(this, &dc);
	}
...
void EditCanvas::removeConnection(EditComponent * node) {
	...
	DrawConnector * drawConnector = nullptr;
	for (std::vector<DrawConnector *>::iterator it = mDrawConnectors.begin(); it != mDrawConnectors.end(); ++it) {
		drawConnector = *it;
...
void EditCanvas::selectComponents() {
	...
	for (std::vector<EditComponent *>::iterator it = allComponents.begin(); it != allComponents.end(); ++it) {
		...
		if (x1 > mStartPositionSelection.x && x2 < mEndPositionSelection.x && y1 > mStartPositionSelection.y && y2 < mEndPositionSelection.y) {
			// Selection box starts in upper left corner and ends in lower right corner
			mSelectedComponents.push_back(component);
		} else if (x1 > mEndPositionSelection.x && x2 < mStartPositionSelection.x && y1 > mEndPositionSelection.y && y2 < mStartPositionSelection.y) {
...
void EditCanvas::moveSelectedComponents(const wxPoint & previousMousePosition, const wxPoint & mp) {
	...
	for (std::vector<EditComponent *>::iterator it = mSelectedComponents.begin(); it != mSelectedComponents.end(); ++it) {
