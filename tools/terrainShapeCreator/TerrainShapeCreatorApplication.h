...
namespace tools {
	...
	public:
		TerrainShapeCreatorApplication(const std::string & goTemplate, const std::string & outFile) : api::Application(), _goTemplate(goTemplate), _outFile(outFile), _manager(nullptr) {
		}

	private:
		std::string _goTemplate;
		std::string _outFile;
		modules::GraphicsManager * _manager;
